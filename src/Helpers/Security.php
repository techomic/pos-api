...
class Security
{
	 ...
	 * @var	string
	 */
	public $charset = $this->container->get('settings')['charset'];
	 ...
	 * @var	int
	 */
	protected $csrfExpire =	$this->container->get('settings')['security']['csrf_expire'];
	 ...
	 * @var	string
	 */
	protected $csrfTokenName =	$this->container->get('settings')['security']['csrf_token_name'];
	 ...
	 * @var	string
	 */
	protected $csrfCookieName =	$this->container->get('settings')['security']['csrf_cookie_name'];
    ...
    public function __construct(Container $container)
    {
        ...
        $csrfProtectionEnabled = $this->container->get('settings')['security']['csrf_protection'];
	...
	public function csrfVerify()
	{
		...
		// // TODO: Check if URI has been whitelisted from CSRF checks
		// if ($excludeUris = $this->container->get('settings')['security']['csrf_exclude_uris']) {
		...
		if ($this->container->get('settings')['security']['csrf_regenerate']) {
		...
		// TODO: log_message('info', 'CSRF token verified');
		return $this;
	}
	...
	public function csrfSetCookie()
	{
		...
		$secureCookie = $this->container->get('settings')['security']['cookie_secure'];

		if ($secureCookie && !$this->container->get(Common::class)->is_https()) {
			return FALSE;
		}
		...
		setcookie(
			...
			$this->csrfHash,
			$expire,
			$this->container->get('settings')['security']['cookie_path'],
			$this->container->get('settings')['security']['cookie_domain'],
			$secureCookie,
			$this->container->get('settings')['security']['cookie_httponly']
		);
		// TODO: log_message('info', 'CSRF cookie sent');
